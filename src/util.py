...
def get_wav_for_df(df, sr):
    ...
    for i, item in enumerate(range(len(df))):
        ...
        signal, _ = librosa.load(
            "../../Data" + df.iloc[i]["path"].strip().strip("'"),
            offset=df.iloc[i]["fine_start_time"],
            duration=df.iloc[i]["fine_end_time"] - df.iloc[i]["fine_start_time"],
            sr=sr,
        )
        x.append(signal)
        ...
        for path_string in path_strings:
            ...
            if path_string in path:
                ...
                if verbose == 2:
                    print(df_wav)
                wav = librosa.load(
                    "../../Data" + df_wav.iloc[0]["path"].strip().strip("'"), sr=8000
                )
                ...
                if sample_end_time.iloc[len(df_wav) - 1] > len(wav[0]):
                    if verbose >= 1:
                        print(
                            "Sample end time",
                            sample_end_time.iloc[len(df_wav) - 1],
                            "greater than length",
                            len(wav[0]),
                            "of the wave file, rounding down label to end of array",
                        )
                ...
                for i in range(len(df_wav)):
                    ...
                    idx_wav_array[
                        np.arange(
                            sample_start_time.iloc[i], sample_end_time.iloc[i]
                        ).astype("int")
                    ] = 1
                x.append(wav[0][np.where(np.logical_not(idx_wav_array))])
                signal_length += (
                    len(wav[0][np.where(np.logical_not(idx_wav_array))]) / sr
                )
        ...
        if len(audio) > 0:
            if feat_type == "mfcc":
                ...
                feat = librosa.feature.melspectrogram(
                    y=np.array(audio), sr=sr, n_mels=n_feat
                )
                # Added case to present features in decibels:
                feat = librosa.power_to_db(feat, ref=np.max)
            else:
                raise Exception(
                    "Feature type of log-mel, mfcc-cut, or mfcc only supported."
                )
            if norm_per_sample:
            ...
            if preds[index] == 1:
                preds_list.append(
                    [
                        ...
                        sample_end / sr,
                        "{:.2f}".format(
                            np.mean(preds_prob[prob_start_idx:prob_end_idx][:, 1])
                        ),
                    ]
                )
...
def detect_timestamps_BNN(
    preds_prob, G_X, U_X, hop_length=512, det_threshold=0.5, sr=8000
):
            ...
            if preds[index] == 1:
                preds_list.append(
                    [
                        ...
                        + " PE: "
                        + "{:.4f}".format(np.mean(G_X[prob_start_idx:prob_end_idx]))
                        + " MI: "
                        + "{:.4f}".format(np.mean(U_X[prob_start_idx:prob_end_idx])),
                    ]
                )
            sample_start = frames[index + 1]
            prob_start_idx = index + 1
